...
--
-- Host: localhost
-- Czas generowania: 20 Kwi 2022, 17:32
...
DROP PROCEDURE IF EXISTS `selectPrzebieg`$$
...
DROP PROCEDURE IF EXISTS `selectUsterki`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectUsterki` ()   SELECT * FROM usterki$$

DROP PROCEDURE IF EXISTS `selectWlasciciel`$$
